...
-   **Garage Door.** Allows you to create a virtual garage door. Generates a HomeKit notification when the accessory's state changes. CarPlay will display the Garage widget on the display when you approach your home. The "obstruction detected" property can be set or reset via a [webhook call](#webhook-service-configuration). If the obstruction sensor is set, per UL 325 safety standards, if the garage door is open it will remain open. If the garage door is opening it will stop and reverse back to open. It will operate as normal in all other circumstances.
